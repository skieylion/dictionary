...
public interface CardRepository extends JpaRepository<Card, Long> {
    @Query(
            "select c from Card c left join fetch c.slots s left join fetch c.transcriptions t where s.id=:slotId")
    List<Card> findAllBySlotId(@Param("slotId") Long slotId, Pageable pageable);

    @Query(
            value =
                    "select distinct on(ce.cardid,max(ce.eventDate)) cr.id\n"
                            + "from CardEvent ce \n"
                            + "left join Card cr\n"
                            + "on cr.id=ce.cardid\n"
    ...
    //    @Query("select cr from Card cr left join cr.slots sl left join cr.cardRemembers rem where
    // sl.id = :slotId and rem.")
    //    List<Card> findDeadlineCardsBySlotId(@Param("slotId") Long slotId);
