...
public class SlotBox extends VerticalLayout {
    ...
    public Collection<Slot> getSelectedSlots() {
        Collection<Long> ids =
                slotBox.getSelectedItems().stream().map(Pair::getFirst).collect(Collectors.toList());
        return slots.stream().filter(slot -> ids.contains(slot.getId())).collect(Collectors.toList());
    }
